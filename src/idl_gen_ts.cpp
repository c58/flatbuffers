...
class TsGenerator : public BaseGenerator {
 ...
 private:
  ...
  static void GenDocComment(std::string* code_ptr,
                            const char* indent = nullptr) {
    GenDocComment(std::vector<std::string>(), code_ptr, indent);
  }
  ...
  void GenObjApi(const Parser& parser, StructDef& struct_def,
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (IsScalar(field.value.type.base_type) || is_string) {
        const auto has_null_default = !HasDefaultValue(field) || HasNullDefault(field);
      ...
      if (!struct_def.fixed) {
        ...
        if (has_create) {
          ...
          if (field.IsScalarOptional()) {
            pack_func_create_call +=
                " && " + field_to_check_null + " !== " + null_keyword_ + ")\n    ";
          } else {
            pack_func_create_call += ")\n    ";
          }
      ...
      if (IsVector(field.value.type)) {
        ...
        if (IsScalar(vectorType.base_type) && !IsLong(vectorType.base_type)) {
          ...
          code += "new " + GenType(vectorType) + "Array(" + GenBBAccess() +
                  ...
                  ".__vector_len(bb_pos + offset)) : " + null_keyword_ + ";\n  }\n\n";
        }
      }
    }
